async def test_generate_happy_path(news_generator, fake_genai_client, sample_article, prompt_repository):
...
async def test_generate_with_no_chat_id_uses_default_prompt(
    ...
    await news_generator.generate(sample_article, chat_id=None)
    _, prompt = fake_genai_client.calls[0]
    assert prompt.startswith("Default:")


async def test_generate_retries_then_succeeds(news_generator, fake_genai_client, sample_article):
